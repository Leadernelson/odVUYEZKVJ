...
class AllDebridService(BaseDebridService):
    ...
    async def _convert_torrent_link(self, magnet: str, api_key: str, season: Optional[str] = None, episode: Optional[str] = None) -> Optional[str]:
        ...
        # 4. Sélectionner le meilleur fichier vidéo
        video_files = [
            f for f in all_files
            if f["filename"].lower().endswith(VIDEO_EXTENSIONS)
        ]
        if not video_files:
            video_files = all_files  # fallback : tous les fichiers

        selected_file = None

        # Séries : chercher l'épisode exact dans un season pack
        if season and episode and len(video_files) > 1:
            try:
                pattern = f"S{int(season):02d}E{int(episode):02d}"
                matching = [
                    f for f in video_files
                    if pattern.upper() in f["filename"].upper()
                ]
                if matching:
        ...
        if not selected_file:
            selected_file = max(video_files, key=lambda f: f["size"])
